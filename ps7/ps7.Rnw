...
while (min >0){
  ...
  error <- c(error, sum((eigen_vals - num_lbda1)^2) )
...
@
When emperical condition number is at 9.951082e+15 magnitude and true condition number is at 1e+22 magnitude, the matrix is not numerically positive semi-definite.\\
Moreover, from above values of errors and condition numbers, we can see that 
(1)the error approximately increases in $10^2$ magnitude as the eigenvalues increase in $10^1$ magnitude, and 
